...
const OBS_MODE=true, CAMERA_MODE=false;
function notifyHost(state){if(window.parent!==window)window.parent.postMessage({type:'anime25d',state},location.origin);}
if(OBS_MODE) document.body.classList.add('obs');
const trackingChannel=null;
...
function status(message,error=false){statusEl.textContent=message;statusEl.classList.toggle('error',error);if(error)notifyHost('error');}
...
if(!gl){ document.getElementById('stage').textContent='WebGL未対応'; notifyHost('error'); return; }
...
async function loadModel(source,name){
  ...
  try{
    ...
    resetParams(); // Blog playback always starts with upstream defaults.
...
const SAMPLE_FILES={A:'neko_fixed6.psd',B:'neko_fixed6.psd'};
...
const auto = { idle:true, blink:true, rand:true, talk:true, mouse:true, mic:false, phys:true, cam:false };
...
window.addEventListener('message',event=>{
  ...
  if(!data||data.type!=='anime25d-pointer')return;
  if(data.active===false){mouse.in=false;return;}
  if(data.active!==true||!Number.isFinite(data.x)||!Number.isFinite(data.y))return;
  mouse.x=clamp(data.x,-1,1);mouse.y=clamp(data.y,-1,1);mouse.in=true;
});
...
function animate(now,dt){
  ...
  if(auto.rand&&!camLive&&!(auto.mouse&&mouse.in)){
...
let hostReady=false, lastRender=0;
function tick(now){
  ...
  render(lastFrame);
  if(!hostReady){hostReady=true;notifyHost('ready');}
  if(capturePending){
