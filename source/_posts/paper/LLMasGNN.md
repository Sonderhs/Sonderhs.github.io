...
## 1️ Graph Understanding Module：用 LLM 模拟 GNN（即“LLM as GNN”）
...
## 2️ Graph Inference Module：构建语言化图词表，支持跨任务/图推理
...
###  LLM 微调：Instruction-based Fine-tuning
...
###  防止幻觉（Hallucination）：Prefix Tree 解码
...
非常好的问题！Graph Vocabulary 是这篇论文的关键创新之一，它解决了图节点表示中的 **OOV 问题**、**语义错位** 和 **迁移性差** 的核心挑战。我们来详细讲一下它的 **构建过程、设计动机、技术细节**。
...
##  构建 Graph Vocabulary 的流程

### 第一步：获取初始节点表示（Initial Representation）
...
### 第二步：语言 ID 归一化（Tokenization + Vocabulary 构建）
...
### 第三步：任务输入中的节点替换（Tokenization 应用）
...
### 第五步：生成时防止幻觉（Constrained Decoding）
...
# Prefix Tree Search（前缀树搜索）

它是为了解决 LLM 在推理时“生成幻觉（hallucination）”的问题。

---

## 什么问题需要 Prefix Tree Search？
...
# 为什么Graph Vocabulary相对于节点ID当token更有助于泛化
