...
cover: ../image/Java/算法Leetcode/Day4_Leetcode27.移除元素/1.JPG
---
...
class Solution {
    public int removeElement(int[] nums, int val) {
        ...
        for(fast=0;fast<nums.length;fast++){
            //当快指针指向的元素不等于val时，说明该元素使我们需要放入新数组的元素
            //则nums[slow] = nums[fast]
            if(nums[fast] != val){
                nums[slow] = nums[fast];
                slow++;
            }
...
```
![动画演示](/image/Java/算法Leetcode/Day4_Leetcode27.移除元素/移除元素.gif)
## 官方答案
### 方法一：双指针
**思路及算法：**
